#Activate libraries 'ape' and 'XML'
library(ape)
library(XML)

### 1) EDIT THE FOLLOWING THREE LINES TO PROVIDE PATHS TO THE fineRADstructure OUTPUT
setwd() ## set working directory
chunkfile <-"path to your fineRADStructure co-ancestry matrix" ## alleles.loci.min2_chunks.out file
mcmcfile <-"path to your fineRADStructure results - individuals assigned to populations" ## finestructure mcmc file .mcmc.xml
treefile<-"path to your fineRADStructure tree file" ## finestructure tree file .mcmcTree.xml
plotsFolder <- "path to your plot folder"
file_name <- "your output file name";  maxIndv <- 10000; maxPop<-10000
final_data <- prepare_fineRAD_data(chunkfile, mcmfile, treefile)
plotFineRAD(final_data) # plot should be saved in your "plotsFolder" directory



prepare_fineRAD_data <- function(chunkfile, mcmfile, treefile) {
  ...
  numeric_indices <- match(fullorder, rownames(dataraw))
  datamatrix <- dataraw[numeric_indices, numeric_indices]
  tmpmat <- datamatrix
  tmpmat[tmpmat > maxIndv] <- maxIndv  # cap the heatmap

  return(list(tmpmat = tmpmat, tdend = tdend, some.colorsEnd = some.colorsEnd))
}

plotFineRAD <- function(final_data) {
  pdf(file = paste(plotsFolder, file_name,"-SimpleCoancestry.pdf", sep = ""), height = 20, width = 20)
  plotFinestructure(final_data$tmpmat, dimnames(final_data$tmpmat)[[1]], dend = final_data$tdend, cols = final_data$some.colorsEnd, cex.axis = 1.1, edgePar = list(p.lwd = 0, t.srt = 90, t.off = -0.1, t.cex = 0.9))
  dev.off()
}
